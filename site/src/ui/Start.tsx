    ...
    >
      ...
      <div className="doors">
        <button type="button" className="card door" onClick={() => setMode('new')}>
          ...
          <span className="door-aside">Creates a new package</span>
        </button>
        <button type="button" className="card door" onClick={() => fileInput.current?.click()}>
          <h3>Open a .mary file</h3>
          <p>
            Import one you already have. Anything this editor does not show is kept exactly as it
            is.
          </p>
          <span className="door-aside">Opens a file you already have</span>
        </button>
      </div>

      <p className={`drop-hint${hot ? ' hot' : ''}`}>
        …or drop a <code>.mary</code> file anywhere on this page.
      </p>
