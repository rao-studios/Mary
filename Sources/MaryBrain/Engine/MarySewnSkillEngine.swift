...
//        Sewn, so an unready server is an error, not a quieter engine. The
//        `choice` is stored because it is WHICH BACKEND Sewn will use, and the
//        behavioral record and the Life gate both read it.
//
import Foundation

public actor MarySewnSkillEngine: InferenceEngine {
    ...
    public init(client: any SewnSkillProviding, choice: LLMEngineChoice = .mistral) {
        self.client = client
        self.choice = choice
        self.displayName = "Skills via Sewn — \(choice.displayName)"
    }

    /// Nothing to warm here: the model, if there is one, lives in Sewn and is
    ...
    private func streamRound(
        ...
        // NO SILENT SECOND ENGINE. On-device now means Sewn's backend, so an
    ...
    /// ride as labeled user text — the same mapping Sewn applies on-device.
    static func messages(from history: [BrainTurn]) -> [SewnChatMessage] {
        ...
        return mapped.map { SewnChatMessage(role: $0.role, content: $0.text) }
    }
}
