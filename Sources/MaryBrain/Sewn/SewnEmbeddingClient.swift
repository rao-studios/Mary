...
/// Injectable POST seam, sibling of `SewnCompleteTransport`.
public protocol SewnEmbeddingTransport: Sendable {
    func post(_ request: URLRequest) async throws -> (status: Int, body: Data)
}

public enum SewnEmbeddingError: LocalizedError, Equatable {
    ...
    public var errorDescription: String? {
        ...
        case .http(let status): return "Sewn embed failed (\(status))."
        case .unreachable(let reason): return "Sewn is unreachable: \(reason)"
        case .undecodable: return "Sewn embed returned an unreadable body."
        case .countMismatch(let sent, let returned):
            return "Sewn embed returned \(returned) vectors for \(sent) texts."
        }
    }
}
...
/// The embedding seam. Tests script it; the app wires `SewnEmbeddingClient`.
public protocol SewnEmbeddingProviding: Sendable {
    func isReady() async -> Bool
    func embed(_ texts: [String]) async throws -> EmbeddedBatch
}

public actor SewnEmbeddingClient: SewnEmbeddingProviding {

    /// Mistral rejects batches larger than this, and Sewn's own provider caps
    ...
    private let transport: any SewnEmbeddingTransport

    public init(
        baseURL: URL,
        session: SewnSession,
        model: String? = nil,
        transport: any SewnEmbeddingTransport = URLSessionEmbeddingTransport()
    ) {
    ...
    private func embedOneBatch(_ texts: [String]) async throws -> EmbeddedBatch {
        guard var token = await session.validToken() else {
            throw SewnEmbeddingError.notAuthenticated
        }
        let body = try JSONEncoder().encode(
            SewnWire.EmbedRequest(inputs: texts, model: model))
        ...
        guard attempt.status == 200 else {
            throw SewnEmbeddingError.http(attempt.status)
        }
        guard let response = try? JSONDecoder().decode(
            SewnWire.EmbedResponse.self, from: attempt.body)
        else { throw SewnEmbeddingError.undecodable }
        ...
        guard ordered.count == texts.count else {
            throw SewnEmbeddingError.countMismatch(
    ...
    private func open(body: Data, bearer: String) async throws -> (status: Int, body: Data) {
        ...
        } catch let error as SewnEmbeddingError {
            throw error
        } catch {
            throw SewnEmbeddingError.unreachable(error.localizedDescription)
        }
    }
}
...
public struct URLSessionEmbeddingTransport: SewnEmbeddingTransport {
