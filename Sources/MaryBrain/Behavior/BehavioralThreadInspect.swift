//
//  BehavioralThreadInspect.swift
//  MaryBrain
//
//  WHAT: Pure shape of a Thread behavioral deposit (tags, codec).
//  IN:   ThreadContextStore writer
//  OUT:  bytes a test can pin without a server
//
import Foundation
import MaryFoundation

public enum BehavioralThreadInspect {

    public static func abilityTags(
        episode: BehavioralEpisode, target: AbilityThreadTarget
    ) -> [String] {
        var tags = [
    ...
    public static func interactionStub(
        ...
        return BehavioralInteractionStub(
            ...
            abilityDocumentID: ThreadMemoryTopology.behaviorDocumentID(episodeID: episode.id),
            abilityGroupIDs: episode.abilityTargets.map {
                ThreadMemoryTopology.abilityGroup(target: $0, ownerID: ownerID).id
            },
...
public struct BehavioralCodecView: Equatable, Sendable {
    ...
    public init(episode: BehavioralEpisode) {
        ...
        sealedReason = episode.sealedReason?.rawValue
        didAct = episode.didAct
        trainingTags = BehavioralThreadInspect.trainingTags(episode: episode)
