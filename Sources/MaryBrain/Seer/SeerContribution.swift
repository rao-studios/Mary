...
public struct SeerContribution: Codable, Sendable, Equatable {
    ...
    public struct Owner: Codable, Sendable, Hashable, Identifiable {
        ...
        public var id: String { ownerID ?? totemID }
        ...
        // Owners are keyed by totem, matching Sis's GitaOwner semantics.
        public func hash(into hasher: inout Hasher) {
            hasher.combine(totemID)
        }

        public static func == (lhs: Owner, rhs: Owner) -> Bool {
            lhs.totemID == rhs.totemID
        }
    }
    ...
    public static func fromJSON(_ json: String) -> SeerContribution? {
        guard let data = json.data(using: .utf8) else { return nil }
        return try? JSONDecoder().decode(SeerContribution.self, from: data)
    }
}
