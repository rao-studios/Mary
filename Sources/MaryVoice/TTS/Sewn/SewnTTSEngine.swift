...
//  MaryVoice
//
//  WHAT: Cloud synthesis via local Sewn /v1/speak (header + float32 PCM).
...
public enum SewnTTSError: Error, LocalizedError, Equatable {
    case notSignedIn
    case badStatus(Int)
    ...
    public var errorDescription: String? {
        switch self {
        case .notSignedIn:
            return "Not signed in to Sewn — its voice needs a session."
        case .badStatus(let code):
            switch code {
            case 401: return "Sewn rejected the session token (401)."
            case 502: return "Sewn couldn't reach Mistral for speech (502)."
            default:  return "Sewn speech failed with HTTP \(code)."
            }
        case .malformedHeader:
            return "Sewn's audio stream was malformed."
        case .emptyAudio:
            return "Sewn returned no audio for the chunk."
        }
    }
}

public actor SewnTTSEngine: SpeechSynthesizer {
    ...
    /// Carries a chunk when Sewn can't (dead server, 401/5xx after retry).
    private var fallback: (any SpeechSynthesizer)?

    public init(
        baseURL: URL,
        character: VoiceCharacter = .marie,
        model: String = SewnTTSEngine.defaultModel,
    ...
    /// Fired once when Sewn succeeds after degradation — re-arm the notice.
    ...
    public func synthesizeChunk(_ text: String) async throws -> SynthesizedChunk {
        ...
        do {
            let chunk = try await synthesizeViaSewn(text)
            noteSewnSuccess()
            return chunk
    ...
    private func isAuthShaped(_ error: Error) -> Bool {
        if case .notSignedIn = error as? SewnTTSError { return true }
        if case .badStatus(401) = error as? SewnTTSError { return true }
        return false
    }
    ...
    private func synthesizeViaSewn(_ text: String) async throws -> SynthesizedChunk {
        ...
        try Task.checkCancellation()
        guard let token else { throw SewnTTSError.notSignedIn }
    ...
    func makeRequest(text: String, voiceID: String, token: String) throws -> URLRequest {
        ...
        // `stream` is decoded but ignored server-side — Sewn always streams.
    ...
    private func performSynthesis(_ request: URLRequest) async throws -> SynthesizedChunk {
        ...
        guard (200...299).contains(http.statusCode) else {
            throw SewnTTSError.badStatus(http.statusCode)
        }
        ...
        let (header, pcm) = try SewnPCMHeader.split(payload)
        let rate = Double(header.sampleRate)
        sampleRate = rate

        var samples = PCMBytes.floats(fromFloat32LE: pcm)
        guard !samples.isEmpty else { throw SewnTTSError.emptyAudio }
    ...
    nonisolated func isRetryable(_ error: Error) -> Bool {
        ...
        if case .badStatus(let code) = error as? SewnTTSError {
...
/// The 8-byte prefix Sewn writes before the PCM: sampleRate UInt32 LE,
/// channels UInt16 LE, bits UInt16 LE (24000 / 1 / 32 today).
struct SewnPCMHeader: Equatable {
    var sampleRate: UInt32
    var channels: UInt16
    var bits: UInt16

    static func split(_ payload: Data) throws -> (header: SewnPCMHeader, pcm: Data) {
        guard payload.count >= 8 else { throw SewnTTSError.malformedHeader }
        let header = payload.withUnsafeBytes { raw -> SewnPCMHeader in
            SewnPCMHeader(
        ...
        guard header.channels == 1, header.bits == 32,
            ...
            throw SewnTTSError.malformedHeader
        }
