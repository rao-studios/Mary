...
//  mary-thread-probe
//
//  WHAT: Headless ThreadDirectClient against live local Thread (:9090).
...
func parseEntities(_ raw: [String]) -> [ThreadEntityIn] {
    raw.compactMap { spec in
        let parts = spec.split(separator: ":", maxSplits: 1).map(String.init)
        guard let name = parts.first, !name.isEmpty else { return nil }
        return ThreadEntityIn(name: name, kind: parts.count > 1 ? parts[1] : "concept")
    }
}

func parseRelations(_ raw: [String]) -> [ThreadRelationIn] {
    raw.compactMap { spec in
        let parts = spec.split(separator: ":").map(String.init)
        guard parts.count == 3 else { return nil }
        return ThreadRelationIn(subject: parts[0], predicate: parts[1], object: parts[2])
    }
}
...
enum Probe {
    static func main() async {
        ...
        guard let command = rawArguments.first else {
            fail("usage: mary-thread-probe <deposit|search|library|groups|stats|remove|roundtrip> …")
        }
        ...
        let client = ThreadDirectClient(host: host, port: port)
        do {
        switch command {
        case "deposit":
        ...
        case "roundtrip":
            ...
            let probeText = "Mary's thread probe verifies the Conduit wire contract end to end."
            let item = DepositItem(
                documentID: documentID,
                texts: [probeText],
                name: "roundtrip probe",
                entities: [
                    ThreadEntityIn(name: "mary", kind: "project"),
                    ThreadEntityIn(name: "thread probe", kind: "skill"),
                ],
                relationships: [
                    ThreadRelationIn(subject: "thread probe", predicate: "verifies", object: "mary")
                ]
            )
            ...
            // Index responds before Thread's write queue drains — the doc is
