//
//  ThreadContextStore+UnitIndex.swift
//  MaryRuntime
//
//  WHAT: Per-project unit-index manifests in Thread.
...
extension ThreadContextStore {
    ...
    func depositUnitIndex(_ unit: IndexedUnit, manifest: UnitIndexManifest) async {
        ...
        let documentID = ThreadMemoryTopology.unitDocumentID(
    ...
    func loadUnitManifest(projectID: String) async -> UnitIndexManifest? {
        ...
        let id = ThreadMemoryTopology.unitManifestID(projectID: projectID, ownerID: owner)
    ...
    private func persistUnitManifest(
        ...
        let item = DepositItem(
            documentID: ThreadMemoryTopology.unitManifestID(
                projectID: projectID, ownerID: ownerID),
            texts: [content],
            tags: ["mary", "project", "unit-manifest"],
            name: "\(projectName) index",
            metadata: Data(),
            entities: [ThreadEntityIn(name: projectName, kind: "project")],
            relationships: [])
