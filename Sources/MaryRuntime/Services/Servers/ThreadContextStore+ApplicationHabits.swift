//
//  ThreadContextStore+ApplicationHabits.swift
//  MaryRuntime
//
...
//  OUT:  ThreadDirectClient deposit / documents
...
extension ThreadContextStore {
    ...
    /// `mary-style-<owner>`, so the Threads pane files it under Personal.
    static func applicationHabitGroup(ownerID: String) -> (id: String, label: String) {
        let group = ThreadMemoryTopology.applicationHabitGroup(ownerID: ownerID)
    ...
    package func rememberApplicationHabits(
        ...
        let item = DepositItem(
            documentID: ThreadMemoryTopology.applicationHabitLedgerDocumentID(
    ...
    package func recallApplicationHabits(
        ...
        guard let documents = try? await client.documents(ids: [id], ownerID: owner),
            ...
            // Thread down, mid-restart, or nothing stored yet: this launch
...
/// The seam MaryBrain reads, filled by the runtime's thread. A thin forwarder
/// rather than a stored client, for the reason `ThreadRoutingHabitMemory`
/// gives: the context store's client is re-pointed when the Thread port
/// changes in Settings, and habits must follow that without reinstalling.
struct ThreadApplicationHabitMemory: ApplicationHabitMemory {
    func remember(_ habits: [ApplicationHabit], discipline: AbilityID) async {
        await MaryRuntime.threadContext.rememberApplicationHabits(
            habits, discipline: discipline)
    }

    func recall(discipline: AbilityID) async -> [ApplicationHabit] {
        await MaryRuntime.threadContext.recallApplicationHabits(discipline: discipline)
    }
}
