//
//  ThreadContextStore+Addressing.swift
//  MaryRuntime
//
//  WHAT: Thread group/document addressing for deposits.
//  OUT:  ThreadMemoryTopology / group ids
//
...
extension ThreadContextStore {
    ...
    /// Durable projections always file to Ability Thread. Destinations come from
    ...
    static func projectionDestinations(
        projection: ResolvedThreadProjection,
        ...
        for target in targets {
            let ability = ThreadMemoryTopology.abilityGroup(
    ...
    static func projectedComposition(
        ...
        if let projectName, !projectName.isEmpty, !names.contains(projectName) {
            composition.entities.append(ThreadEntityIn(name: projectName, kind: "project"))
        ...
        if projection.purpose == .content,
           ...
           !knowledge.value.isEmpty {
            ...
            if !names.contains(concept) {
                composition.entities.append(ThreadEntityIn(name: concept, kind: "concept"))
                names.insert(concept)
            }
            if let projectName, names.contains(projectName) {
                composition.relationships.append(ThreadRelationIn(
                    subject: projectName,
    ...
    package static func metadata(
        ...
        bindingName: String,
        succeeded: Bool = true,
        projection: ResolvedThreadProjection? = nil,
        lane: ThreadLane? = nil
    ) -> Data {
        ...
        if let lane { fields["thread_lane"] = lane.rawValue }
