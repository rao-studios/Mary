//
//  ThreadGraphPolicy.swift
//  MaryRuntime
//
//  WHAT: Mary's ontology kinds + co-mention auto-edges on Thread's graph policy.
//  OUT:  GET → merge → PUT on Thread HTTP `/v1/graph/policy`
//  PIN:  Raw JSON merge so unknown fields survive. Idempotent. Prospective-only.
//

import Foundation

package enum ThreadGraphPolicy {
    ...
    package static func push(threadPort: Int) async -> Bool {
        guard let url = URL(string: "http://127.0.0.1:\(threadPort)/v1/graph/policy") else {
            return false
        }
