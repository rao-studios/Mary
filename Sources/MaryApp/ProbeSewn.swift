...
//  Mary
//
//  WHAT: Headless full-stack Sewn turn (servers, sign-in, dual-lane brain).
//  OUT:  contribution + timing. CLI: swift run Mary --probe-sewn "…" [--follow] [--then]
//  PIN:  Always wires Sewn, even if Brain card says on-device.
//
...
enum ProbeSewn {
    ...
    static func shouldRun() -> Bool {
        CommandLine.arguments.contains("--probe-sewn")
    }
    ...
    private static func run() async -> Int32 {
        ...
        guard let flagIndex = arguments.firstIndex(of: "--probe-sewn"),
              flagIndex + 1 < arguments.count else {
            print("Usage: Mary --probe-sewn <text> [--engine mistral|local|tinker] [--transport classic|realtime] [--speak]")
            return 1
        }
        ...
        var transport = SewnTransportChoice.classic
        if let transportIndex = arguments.firstIndex(of: "--transport"), transportIndex + 1 < arguments.count {
            guard let parsed = SewnTransportChoice(rawValue: arguments[transportIndex + 1]) else {
        ...
        defaults.sewnChatModel = chatModel
        ...
        let policyPushed = await ThreadGraphPolicy.push(threadPort: defaults.threadPort)
        ...
        // WHICH BACKEND SEWN USES; every lane rides Sewn either way, and
        // applyEngine is what sets the boxes connectSewnToBrain's rewire reads.
        ...
        if speak {
            ...
            if let notice = await MaryRuntime.applyTTSBackend(
                .sewn, hostedVoice: VoiceCharacter.marie.id) {
                print("(\(notice))")
            } else {
                print("[tts] speaking through Sewn /v1/speak")
            }
        }
        let projects = ["mary": FileManager.default.currentDirectoryPath]
        await MaryRuntime.installBrainConfiguration(projects: projects)
        // THE PROBE ALWAYS WIRES SEWN, whatever the Brain card says — it
        ...
        await MaryRuntime.applySewnTransport(transport)
    ...
    private static func runTurn(text: String, speak: Bool) async throws {
        ...
        for try await event in events {
            ...
            case .contribution(let json):
                if let contribution = SewnContribution.fromJSON(json) {
                    print("\n[contribution] \(contribution.owners.count) owner(s)")
                    for ownerEntry in contribution.owners {
                        print("  thread=\(ownerEntry.threadID.prefix(8))… royalty=\(ownerEntry.royalty) spans=\(ownerEntry.spans.map { "\($0.lower)..\($0.upper)" }.joined(separator: ",")) docs=\(ownerEntry.documentIDs.count)")
                    }
                }
            case .autoMemoryTriggered:
