...
struct ThreadsGraphView: View {
    ...
    private enum PendingRepair: Equatable {
        ...
        var message: String {
            ...
            case .reextract:
                return "Thread replays its LLM extractor over the document — this costs model calls and can take up to two minutes."
            }
        }
        ...
        VStack(alignment: .leading, spacing: .layer3) {
            ...
            if vm.isMutating {
                HStack(spacing: .layer2) {
                    ...
                    Text("Asking Thread to mutate the graph…")
                        .font(.marySans(11))
    ...
    private var controls: some View {
        VStack(alignment: .leading, spacing: .layer2) {
            ...
            FlowLayout(spacing: 4, lineSpacing: 4) {
                ForEach(ThreadGraphPolicy.maryKinds, id: \.name) { kind in
                    MaryChip(
                        label: kind.name,
    ...
    private func edgeRow(_ edge: GraphRelationship, in graph: GraphQueryResult) -> some View {
        HStack(spacing: .layer2) {
            ...
            Text("\(ThreadExplorerViewModel.entityName(edge.subjectID, in: graph)) —\(edge.predicate)→ \(ThreadExplorerViewModel.entityName(edge.objectID, in: graph))")
                .font(.marySans(10))
    ...
    private var repairsDisabled: Bool {
        !vm.isThreadHealthy || vm.isMutating
    }
    ...
    private func repairs(for entity: GraphEntity) -> some View {
        VStack(alignment: .leading, spacing: .layer2) {
            ...
            if !vm.isThreadHealthy {
                Text("Thread isn't running — repairs need its HTTP port.")
                    .font(.marySans(9))
                    .foregroundStyle(Color.maryInk.opacity(0.45))
            }
        }
