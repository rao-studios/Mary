//
//  ThreadsLibraryView.swift
//  Mary
//
//  WHAT: Thread holdings by lane/family; drill replaces the listing (narrow column).
//
...
struct ThreadsLibraryView: View {
    ...
    private var listing: some View {
        ...
        if let hint = vm.abilityDepositHint,
           laneFilter == nil || laneFilter == ThreadLane.ability.rawValue {
            Text(hint)
                .font(.marySans(11))
        ...
        FlowLayout(spacing: 4, lineSpacing: 4) {
            MaryChip(label: "Ability", isOn: laneFilter == ThreadLane.ability.rawValue) {
                toggleFilter(ThreadLane.ability.rawValue)
            }
            MaryChip(label: "Personal", isOn: laneFilter == ThreadLane.personal.rawValue) {
                toggleFilter(ThreadLane.personal.rawValue)
            }
        ...
        let sections = ThreadExplorerViewModel.sections(
        ...
        if vm.isLoadingLibrary {
            ...
            Text("Nothing here yet. Groups appear as Mary deposits into the thread.")
                .font(.marySans(11))
                .foregroundStyle(Color.maryInk.opacity(0.5))
                .padding(.top, .layer4)
        }
    ...
    private func laneSection(_ section: ThreadLaneSection) -> some View {
    ...
    private func groupCard(_ group: ThreadGroupRow) -> some View {
    ...
    private func documentRow(_ document: ThreadDocumentRow) -> some View {
        Button {
    ...
    private func documentDetail(_ document: ThreadDocumentDetail) -> some View {
        VStack(alignment: .leading, spacing: .layer3) {
            Button {
                ...
                VStack(alignment: .leading, spacing: .layer2) {
                    HStack(spacing: .layer2) {
                        Text(document.name)
                            ...
                            .lineLimit(2)
                        MaryBadge(text: ThreadExplorerViewModel.familyTitle(document.family))
                        Spacer()
                    }
