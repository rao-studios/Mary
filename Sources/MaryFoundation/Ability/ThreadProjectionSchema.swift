//
//  ThreadProjectionSchema.swift
//  MaryFoundation
//
//  WHAT: Which Skill/Interaction fields may persist, and for how long.
//  IN:   `.mary` threadProjections[] → AbilityPackageValidator+Schemas.
//  OUT:  Ability Thread (this Ability + extended disciplines).
//

import Foundation

public enum ThreadLane: String, Codable, Hashable, Sendable, CaseIterable {
    case ability
    case personal
}

/// Ability Thread address: one Ability, one role.
public struct AbilityThreadTarget: Codable, Hashable, Sendable, Comparable {
    ...
    public static func < (lhs: AbilityThreadTarget, rhs: AbilityThreadTarget) -> Bool {
...
/// Machine job of a Thread projection. Closed so content cannot collapse into a redacted receipt.
public enum ThreadProjectionPurpose: String, Codable, Hashable, Sendable, CaseIterable {
    case receipt
    case content
    case interaction
}

public struct ThreadProjectionSchema: Codable, Hashable, Sendable, Identifiable {
