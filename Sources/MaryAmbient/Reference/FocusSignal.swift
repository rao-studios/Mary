...
//  lead box.
//
//  The tracker's `leadBox` holds ONE `(realm, at)` and last-writer-wins;
...
//  realms silently degrade to one-line mentions and a glance at another
//  window (a Look) is no signal at all.
//
//  The ledger keeps ONE freshest evidence stamp per realm, written by the
...
//  `noteGlance` — a successful look_at_screen at that realm's app. A glance
...
//  glanced}; when only one realm has evidence it answers byte-identically to
//  `leadRealm()` — the parity rule the pinned focus tests stand on.
//
...
/// The kind of evidence a realm holds in the ledger, in responder-precedence
...
public enum FocusEvidenceKind: Int, Sendable, Comparable, Equatable {
    /// A watcher saw REAL WORK in the realm (an edit, a document change).
    case activity = 2
    /// The realm's application was activated / came frontmost.
    case activation = 1
    /// A successful look_at_screen at the realm's app — sight, not presence.
...
/// realms with fresh evidence beside it, and which of those are only glanced.
public struct FocusSignal: Sendable, Equatable {
    public var lead: AmbientRealm?
    /// Fresh non-lead realms, strongest-evidence-then-recency ranked.
    public var coActive: [AmbientRealm]
    /// The subset of `coActive` whose freshest evidence is a glance.
    public var glanced: Set<AmbientRealm>

    public init(
        lead: AmbientRealm? = nil,
        coActive: [AmbientRealm] = [],
        glanced: Set<AmbientRealm> = []
    ) {
        self.lead = lead
    ...
    /// How long non-lead activity/activation evidence keeps a realm
...
/// THE ONE bundleID → realm LADDER, extracted from `record(bundleID:)`'s
/// arms so the Look faculty (and anything else holding only a bundle id)
/// resolves realms by the same rules the focus signal uses. The tracker's
...
public enum AmbientRealmResolver {
    ...
    /// The browser workspace realm — Safari, Chrome, and any Chromium
    /// variant sharing their prefixes all resolve to this one workspace.
    public static var browserRealm: AmbientRealm {
        AmbientRealm(world: .applications, application: browserApplicationID)
    }
    ...
    ///
    /// KINDS CLOSED, INSTANCES OPEN — the rule this file's own realm carve-out
    /// is built on, applied to the roster rather than only to the realm. The
    /// browser WORKSPACE is closed vocabulary and stays one realm. The set of
    ...
    public static func evidencedBrowserName() -> String? {
        WorkspaceFocusTracker.shared
            .evidenceProcess(for: browserRealm)
            .flatMap(browserName(bundleID:))
    }

    /// Called on every focus record and every realm resolution, so it answers
    ...
    /// The realm a bundle id belongs to: the five native workspace arms,
    ...
    /// drives it — pulling Chrome into a `.dynamic("chrome")` realm would
    ...
    /// workspace.
    public static func realm(forBundleID bundleID: String) -> AmbientRealm {
    ...
    /// THE IDENTITY-BEARING LADDER — the cursor-obvious lead's realm. Same
    /// rungs as `realm(forBundleID:)` except the terminal: a generic app
    ...
    /// `AmbientApplicationDirectory` via `AmbientRealm.displayName`.
    /// `realm(forBundleID:)` stays byte-identical — it is the FACT lane
    ...
    public static func applicationRealm(forBundleID bundleID: String) -> AmbientRealm {
        let shared = realm(forBundleID: bundleID)
        if shared == .world(.applications) {
            return AmbientRealm(world: .applications, application: bundleID)
        }
        return shared
    }
}
