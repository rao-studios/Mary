...
//  PIN:  A thread search returns documentID, text and score — and NOT the
...
@Suite struct RoutingHabitAddressTests {
    ...
    @Test func aLessonSurvivesTheRoundTripThroughItsDocumentID() throws {
        ...
        let id = ThreadContextStore.RoutingHabitAddress.documentID(for: stored)
        let recalled = try #require(ThreadContextStore.RoutingHabitAddress.habit(
    ...
    @Test func theSeparatorCannotOccurInsideTheParts() {
        let id = ThreadContextStore.RoutingHabitAddress.documentID(for: RoutingHabit(
    ...
    /// A THREAD HOLDS MORE THAN ROUTING MEMORY. Anything that is not one of
    ...
    func aForeignDocumentTeachesNothing(_ documentID: String) {
        #expect(ThreadContextStore.RoutingHabitAddress.habit(
            documentID: documentID, text: "some text") == nil)
    }

    /// THE PANE HAS TO RECOGNISE THEM. `ThreadAddressClassifier` reads families
    ...
    @Test func routingAddressesClassifyOntoThePersonalLane() {
        let group = ThreadAddressClassifier.classifyGroup(
            id: ThreadContextStore.routingHabitGroup(ownerID: "owner-a").id)
        #expect(group.family == .routingGroup)
        #expect(group.lane == .personal)
        #expect(!group.isSewnOwned)

        let document = ThreadAddressClassifier.classifyDocument(
            id: ThreadContextStore.RoutingHabitAddress.documentID(for: RoutingHabit(
    ...
    @Test func theGroupIsScopedToItsOwner() {
        let mine = ThreadContextStore.routingHabitGroup(ownerID: "owner-a")
        let theirs = ThreadContextStore.routingHabitGroup(ownerID: "owner-b")
