...
final class RequestBuildingTests: XCTestCase {
    ...
    func testIndexRequestMapsAllFields() {
        let items = [
            DepositItem(
                ...
                entities: [
                    ThreadEntityIn(name: "ritesh", kind: "person"),
                    ThreadEntityIn(name: "mary", kind: "project"),
                ],
                relationships: [
                    ThreadRelationIn(subject: "ritesh", predicate: "works on", object: "mary")
                ]
            )
        ]
    ...
    func testGraphBrowseRequestLeavesEntityAndQueryEmpty() {
        let request = ThreadProtoMap.graphBrowseRequest(ownerID: "owner-1", limit: 8)
        XCTAssertEqual(request.ownerID, "owner-1")
    ...
    func testGraphResultMapsEntitiesRelationshipsDocumentsAndCounts() {
        var seed = Thread_V1_ThreadGraphEntity()
        seed.id = "e1"
        seed.name = "mary"
        seed.kind = "project"
        seed.score = 0.5
        ...
        var neighbor = Thread_V1_ThreadGraphEntity()
        neighbor.id = "e2"
        ...
        var edge = Thread_V1_ThreadGraphRelationship()
        edge.id = "r1"
        edge.subjectID = "e2"
        ...
        var doc = Thread_V1_ThreadGraphDocument()
        doc.id = "d2"
        ...
        let result = ThreadProtoMap.graphResult(from: response)
