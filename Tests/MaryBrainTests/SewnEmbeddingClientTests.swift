//
//  SewnEmbeddingClientTests.swift
//  MaryBrainTests
//
...
@Suite struct SewnEmbeddingClientTests {
    ...
    @Test func aMissingVectorIsAnError() async {
        ...
        await #expect(throws: SewnEmbeddingError.countMismatch(sent: 3, returned: 2)) {
    ...
    @Test func anUnreadableBodyIsReported() async {
        ...
        await #expect(throws: SewnEmbeddingError.undecodable) {
    ...
    @Test func anErrorStatusIsReported() async {
        ...
        await #expect(throws: SewnEmbeddingError.http(503)) {
            try await client.embed(["first"])
        }
    }
    ...
    @Test func aLargeCorpusIsChunked() async throws {
        let texts = (0..<(SewnEmbeddingClient.maximumBatch + 5)).map { "text \($0)" }
    ...
    private static func client(_ transport: any SewnEmbeddingTransport) -> SewnEmbeddingClient {
        SewnEmbeddingClient(
    ...
    private actor ScriptedTransport: SewnEmbeddingTransport {
        let status: Int
        let body: Data
    ...
    private actor ChunkEchoTransport: SewnEmbeddingTransport {
        private(set) var calls = 0
        ...
        func post(_ request: URLRequest) async throws -> (status: Int, body: Data) {
            ...
            return (200, SewnEmbeddingClientTests.body(model: "mistral-embed", vectors: vectors))
        }
    }
}
