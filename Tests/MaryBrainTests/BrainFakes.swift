...
enum BrainFakes {
    ...
    final class StubDispatcher: AbilityDispatching, @unchecked Sendable {
        ...
        /// Tools whose dispatch reports `landed: true` — the asked-for change is
        /// PROVEN to have happened, which is the one receipt the continuation
        ...
        /// What `projectRoster()` reports as the turn's arbitration. `.empty` by default
        /// — set it to prove a watcher was handed THIS projection rather than one it
        /// re-derived for itself.
        var rosterTrace: AbilityRosterTrace = .empty
        ...
        var abilityRosterTrace: AbilityRosterTrace {
            lock.lock(); defer { lock.unlock() }
            return rosterTrace
        }
