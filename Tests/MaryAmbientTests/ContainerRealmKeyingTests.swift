//
//  ContainerRealmKeyingTests.swift
//  BonnieAmbientTests
//
...
//  applications' documents would collide. Keying by `AmbientRealm.token` gives
//  each registration its own `applications:<id>|…` namespace while every native
//  key stays byte-identical (`token == rawValue` for a native realm).
//
...
@Suite struct ContainerRealmKeyingTests {

    // MARK: - Dynamic identity

    /// A registered application can mint a handle of its own, and the handle
    /// resolves back to the application's realm — not to a bare host lane.
    @Test func aRegisteredApplicationMintsItsOwnHandles() {
        let registry = ContainerRegistry()
        let handle = registry.handle(
            realm: .dynamic("sketch"), prefix: "A", key: "canvas-1")
        #expect(!handle.isEmpty)

        let found = registry.resolveRealm(handle)
        #expect(found?.realm == .dynamic("sketch"))
        ...
        // Idempotent by (realm, key), exactly as native minting is.
        #expect(registry.handle(
            realm: .dynamic("sketch"), prefix: "A", key: "canvas-1") == handle)
    }

    /// A registered application can enroll a listing under its own realm, and
    /// the listing is the application's — not the host lane's.
    @Test func aRegisteredApplicationEnrollsAListing() {
        let registry = ContainerRegistry()
        registry.noteListing(
            realm: .dynamic("sketch"), keys: ["canvas-1", "canvas-2"])

        let remembered = registry.lastListing(for: AmbientRealm.dynamic("sketch"))
        #expect(remembered?.keys == ["canvas-1", "canvas-2"])
        #expect(remembered?.realm == .dynamic("sketch"))
        ...
        #expect(registry.listing(
            for: AmbientRealm.dynamic("sketch"),
        ...
        #expect(registry.evidence(
            realm: .dynamic("sketch"), key: "canvas-1")?.kind == .shown)
    ...
    /// COLLISION-FREE, which means it carries the lane prefix a dynamic realm
    ...
    @Test func evidenceKeysCarryTheWholeRealm() {
        #expect(ContainerRegistry.evidenceKey(
            realm: .dynamic("textedit"), key: "/tmp/todo.txt")
            == "applications:textedit|/tmp/todo.txt")
        #expect(ContainerRegistry.evidenceKey(
            realm: .dynamic("notes"), key: "/tmp/todo.txt")
        ...
        let registry = ContainerRegistry()
        let handle = registry.handle(realm: .dynamic("textedit"), prefix: "W", key: "/tmp/todo.txt")
        #expect(handle == "W1")
        // Realm-form minting for the same native container reuses it — the
        // scoped identifiers are the same bytes, so the map cannot tell the
        // two spellings apart.
        #expect(registry.handle(
            realm: .dynamic("textedit"), prefix: "W", key: "/tmp/todo.txt") == "W1")

        // The dynamic namespace is the prefixed one; native never wears it.
        #expect(ContainerRegistry.evidenceKey(
            realm: .dynamic("sketch"), key: "canvas-1")
    ...
    @Test func twoRegisteredApplicationsContainersStayDisjoint() {
        ...
        let sketch = registry.handle(realm: .dynamic("sketch"), prefix: "A", key: "canvas-1")
        let figma = registry.handle(realm: .dynamic("figma"), prefix: "A", key: "canvas-1")
        #expect(sketch != figma)
        #expect(registry.resolveRealm(sketch)?.realm == .dynamic("sketch"))
        #expect(registry.resolveRealm(figma)?.realm == .dynamic("figma"))

        // Evidence stamped for one never answers for the other.
        registry.noteEvidence(realm: .dynamic("sketch"), key: "canvas-1", .actedOn)
        #expect(registry.evidence(
            realm: .dynamic("sketch"), key: "canvas-1")?.kind == .actedOn)
        #expect(registry.evidence(realm: .dynamic("figma"), key: "canvas-1") == nil)

        // Listings are per-application, not per host lane.
        registry.noteListing(realm: .dynamic("sketch"), keys: ["canvas-1"])
        registry.noteListing(realm: .dynamic("figma"), keys: ["board-1"])
        #expect(registry.lastListing(
            for: AmbientRealm.dynamic("sketch"))?.keys == ["canvas-1"])
        #expect(registry.lastListing(
            for: AmbientRealm.dynamic("figma"))?.keys == ["board-1"])
        ...
        #expect(registry.salienceRanks(
            realm: .dynamic("figma"), keys: ["canvas-1"]).isEmpty)
    }
}
