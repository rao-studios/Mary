//
//  AmbientPlaceABITests.swift
//  BonnieAmbientTests
//
//  THE TOKEN ABI, PINNED BEFORE THE REALM MIGRATION MOVES ANYTHING. These
//  goldens are invariants 2/4/9 of the AmbientPlace plan made executable:
...
//  `.applications` (the audit that licenses `AmbientPlace.application(String)`
...
struct AmbientPlaceABITests {
    ...
    @Test func everyNativeTokenIsItsRawValue() {
        for world in AmbientWorld.allCases {
            #expect(AmbientPlace.lane(world).token == world.rawValue)
        }
    }
    ...
    @Test func registeredTokensRideTheHostLane() {
        AmbientApplicationIndexProvider.$scoped.withValue(Self.roster) {
            #expect(AmbientPlace(world: .applications, application: "sketch").token
                == "applications:sketch")
            #expect(AmbientPlace(world: .applications, application: "ghost").token
                == "applications:ghost")
        }
    }
    ...
    @Test func keyIDsAreByteStable() {
        ...
        let application = AmbientKey(place: .application("pages"), slot: .file)
        ...
        let lane = AmbientKey(place: .lane(.typer), slot: .file)
    ...
    /// lets the place collapse (world, application?) into two enum cases.
    @Test func registrationPlacesFixTheHostLane() {
        let dynamic = ApplicationRegistration(
            ...
            bundleIdentifiers: ["com.bohemiancoding.sketch3"],
            worldClass: .dataSource)
        #expect(dynamic.place == AmbientPlace(world: .applications, application: "sketch"))
        let native = ApplicationRegistration(
            ...
            legacyWorld: nil)
        #expect(native.place == AmbientPlace.application("pages"))
        #expect(native.place.application == "pages",
                "every application is a dynamic place carrying its own id")
    }

    /// The bare persisted spelling: the memory graph stores "pages" and
    /// "sketch" as peers with NO prefix — the place's memory token must keep
    ...
    @Test func everyLaneRoundTripsThroughItsToken() {
        for world in AmbientWorld.allCases {
            ...
            #expect(AmbientPlace.from(token: place.token) == place, "\(world)")
            #expect(AmbientWorld.from(pluginOwner: world.pluginOwner) == world, "\(world)")
        }
    }

    /// AND SO DOES EVERY APPLICATION. A dynamic place's token carries the
    ...
    @Test func applicationsRoundTripAndPersistBare() {
        let place = AmbientPlace.application("textedit")
        #expect(AmbientPlace.from(token: place.token) == place)
    ...
    @Test func orderKeepsNativesFirst() {
        AmbientApplicationIndexProvider.$scoped.withValue(Self.roster) {
            ...
            for world in AmbientWorld.allCases {
                #expect(AmbientPlace.lane(world).order == world.order)
                #expect(registered.order > world.order)
            }
        }
    }

    /// THE MEMORY-GRAPH SPELLING, as its own golden: native places deposit
    /// their bare rawValue and dynamic places their bare id — peers, no
    ...
    @Test func memoryTokensStayBare() {
        for world in AmbientWorld.allCases {
            #expect(AmbientPlace.lane(world).memoryToken == world.rawValue)
        }
        #expect(AmbientPlace.application("sketch").memoryToken == "sketch")
        #expect(AmbientPlace.application("ghost").memoryToken == "ghost")
    }

    /// `from(token:)` is the exact inverse of `token`: every place round-trips,
    ...
    @Test func tokensRoundTrip() {
        for world in AmbientWorld.allCases {
            #expect(AmbientPlace.from(token: AmbientPlace.lane(world).token)
                == .lane(world))
        }
        #expect(AmbientPlace.from(token: AmbientPlace.application("sketch").token)
            == .application("sketch"))
        #expect(AmbientPlace.from(token: "applications:read:doc") == .application("read:doc"),
                "the first colon is the split; the id keeps the rest")
        #expect(AmbientPlace.from(token: "not-a-world") == nil)
        #expect(AmbientPlace.from(token: "pages:sketch") == nil,
                "only the host lane spells dynamics")
        #expect(AmbientPlace.from(token: "applications:") == nil,
                "an empty id is not an identity")
        #expect(AmbientPlace.from(token: "applications") == .lane(.applications),
